    ...
    if (i==0){
      ...
      ReactDOM.findDOMNode(this.refs.list).style.display = "none";
        ...
        axios.get(url).then(function (response){
          ReactDOM.findDOMNode(self.refs.list).style.display = "block";
          if (i == 1) {
            self.setState({option:'Named Entities'});
            self.setState({list:response.data[0]["Paragraphs:"][0]["Named Entities:"]});
          }
          if (i == 2){
            self.setState({option:'Noun Phrases'});
            self.setState({list:response.data[0]["Paragraphs:"][0]["Noun Phrases:"]});
          }
